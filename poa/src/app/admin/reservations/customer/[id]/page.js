...
export default function ReservationsContent({params}) {
  ...
  const getReservations = async () => {
    const response = await fetch(`/api/catalog/reservation/customer/${id}`, {
      method: "GET",
    });
  ...
  return (
    <div className={styles.page}>
      <div style={{display:"flex", justifyContent:"space-between"}}>
        <h1>Order Requests from {id.split("%20").join(" ")}</h1>
        </div>
