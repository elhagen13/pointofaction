...
export default function SetQuantity({
  ...
  const [origOrderQuant, setOrigOrderQuant] = useState([]);

  const [orderPulled, setOrderPulled] = useState([])
  ...
  useEffect(() => {
    ...
    for (const item of prevCart) {
      if (item.style === items[0].style && 
        ...
        previouslyPulled[item.size] = item.pulled
      }
    }
    const arr = [];
    ...
    let orderPulledArr = arr.map(([size]) => {
      return previouslyPulled[size] || 0
    })
    console.log(tempSizeToItemMap)

    setOrderPulled(orderPulledArr)
    setOrigOrderQuant(orderQuantArr)
          ...
          >
            ...
            >
              {sizes.map((available, index) => (
                <div key={index} style={{ padding: "0 10px" }}>
                  {parseInt(available[1].available) + origOrderQuant[index]}
                </div>
              ))}
            </div>
            <div
