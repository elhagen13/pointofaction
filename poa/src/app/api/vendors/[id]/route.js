...
const DATABASE_NAME = process.env.DATABASE_NAME;const COLLECTION_NAME = 'vendors';
...
export async function PATCH(request, { params }) {
  try {
    ...
    const existingVendorItem = await collection.findOne({ _id: new ObjectId(id) });
    ...
    if (body.category !== undefined) {
        updateDocument.category = body.category;
      }
    if (body.link !== undefined) {
      updateDocument.link = body.link.trim();
    }
    if (body.blocked !== undefined) {
        updateDocument.blocked = body.blocked;
      }
      ...
      return Response.json(
        {
          ...
          data: existingVendorItem
        }
      );
    }

    // Return the updated document
    const updatedVendorItem = await collection.findOne({ _id: new ObjectId(id) });
      ...
      return Response.json(
        {
          ...
          details: 'A vendor item with this information already exists'
        },
        { status: 409 }
      );
    }
