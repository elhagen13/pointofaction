  ...
  try {
    ...
    if (!ObjectId.isValid(id)) {
      return Response.json(
        {
          success: false,
          error: 'Invalid description ID format'
        },
        { status: 400 }
      );
    }
    ...
    const existingDescription = await collection.findOne({ _id: new ObjectId(id) });
    ...
    const updatedDescription = await collection.findOne({ _id: new ObjectId(id) });
