...
export default function ProductCard({ box = null, item = null, setSelectedBox, boxDict }) {
    ...
    const getDescription = () => {
        ...
        if (item) {
            setDescription(`${item[0].color} ${item[0].brand} ${item[0].description}`)
            return;
        }
        else if (box) {
    ...
    const getPrice = () => {
        ...
        if (item) {
            ...
            setDiscountedPrice(minPrice !== maxPrice ? `${minPrice}-$${maxPrice}` : minPrice)
            return;
        }
    ...
    return (
        <div className={styles.productCardContainer}>
            {box !== null && <div className={styles.imageList}>
            ...
            <div className={styles.productCard}>
                <div className={styles.imageContainer}>
                    ...
                    {box !== null && <div className={styles.imagePageContainer}>
                        ...
                        <div className={styles.imagePage}>
                            {
                                images.map((image, index) => (
                                    box !== null && <div className={`${styles.imageDots} ${index === currentImage && styles.imageDotsActive}`}
                                        onClick={() => setCurrentImage(index)}
                                    />
                                ))
                            }
                        </div>
                        <div className={styles.nav} onClick={() => setCurrentImage(currentImage + 1 >=
                ...
                <div className={styles.details}>
                    ...
                    <div className={styles.productCardFooter}>
                        <div className={styles.prices}>
                            {box !== null && <div style={{ color: "#a80f0fff", textDecoration: "line-through" }}>${price}</div>}
                            <div style={{ color: "#2c4cb4ff" }}>${discountedPrice}</div>
                        </div>
                        <button className={styles.view} onClick={() => setSelectedBox(box ?? boxDict[item[0].boxId] ?? item[0])}
                        >View</button>
                    </div>
                </div>
            </div>
